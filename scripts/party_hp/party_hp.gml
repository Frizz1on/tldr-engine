...
if instance_exists(o_enc){
    if o_enc.party_state[party_get_index(party_name)] == PARTY_STATE.DEFEND {
        
        if party_getdata(party_name, "highguard_turns") > 0 {
            // nerfs stacking high guard and defending
            hurt *= 3/4;
        }
        else {
            hurt *= 2/3;
...
if party_getdata(party_name, "highguard_turns") > 0 {
    ...
    var guard_power = party_getdata(party_name, "highguard_power");
    
    hurt -= guard_power;

    // enforce minimum scaling based on chapter
    if (hurt <= 5 * save_get("chapter")) {
        hurt += random(12 * save_get("chapter"));
    }
}
