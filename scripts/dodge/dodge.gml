...
/// @desc draw_self but DOES NOT darken anymore
/// @arg {function|undefined} drawer the function that draws your object
function dodge_darken_self(drawer = undefined) {
    if !instance_exists(o_dodge_controller)
        return false

    // Just draw normally — fog shader handles darkness now
    if !is_undefined(drawer) && is_callable(drawer)
        drawer(sprite_index, image_index, x, y, image_xscale, image_yscale, image_angle, c_white, 1)
    else
        draw_self();
}
...
function dodge_gameover(){
    ...
    instance_create(o_gameover, 
        o_dodge_soul.x - guipos_x(), 
        o_dodge_soul.y - guipos_y(), 
        DEPTH_ENCOUNTER.UI,
        {
            image_blend: o_dodge_soul.image_blend,
            freezeframe: sprite_create_from_surface(application_surface, 0, 0, 640, 480, 0, 0, 0, 0),
            freezeframe_gui: sprite_create_from_surface(
                (instance_exists(o_ui_menu) ? o_ui_menu.surf : -1), 
                0, 0, 640, 480, 0, 0, 0, 0
            ),
        }
    )
