...
if (pattern == "laser_lock") {
    ...
    for (var _i = 0; _i < array_length(_lock_times); _i++) {
        ...
        if (timer == _t_warn) {
            ...
            _w.image_xscale = 0.7; _w.image_yscale = 0.7;
            _w.alarm[0] = 15;   // self-destruct after warn window
            // Raycast warning line
            var _ray = instance_create_depth(lock_x, _bt,
                DEPTH_ENCOUNTER.BULLETS_OUTSIDE, o_enc_bullet_raycast);
            _ray.ray_angle = 270; _ray.ray_length = o_enc_box.height + 16;
            _ray.ray_life = 14;  _ray.ray_width = 2;
            array_push(ray_insts, _ray);
        }
        if (timer == _t_fire) {
            for (var _r = 0; _r < array_length(ray_insts); _r++)
                if instance_exists(ray_insts[_r]) instance_destroy(ray_insts[_r]);
            ray_insts = [];
...
/// PING CROSS
    ...
    if (timer == 16 || timer == 44 || timer == 72) {
        ...
        _wl.image_angle = _aim_l; _wl.alarm[0] = 13;
        ...
        _wr.image_angle = _aim_r; _wr.alarm[0] = 13;
    }
    if (timer == 28 || timer == 56 || timer == 84) {
        ...
        for (var _k = -1; _k <= 1; _k++) {
            var _blt = instance_create_depth(_bl - 8, _by + _k * 10,
                DEPTH_ENCOUNTER.BULLETS_OUTSIDE, o_enc_bullet);
            _blt.direction = _aim_l2 + _k * 8; _blt.speed = 3.7; _blt.image_angle = _blt.direction;
            var _brt = instance_create_depth(_br + 8, _by + _k * 10,
                DEPTH_ENCOUNTER.BULLETS_OUTSIDE, o_enc_bullet);
            _brt.direction = _aim_r2 - _k * 8; _brt.speed = 3.7; _brt.image_angle = _brt.direction;
        }
    }
    if (timer >= 104) {
        o.depth_override = undefined;
